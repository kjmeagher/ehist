...
class TestAxis(unittest.TestCase):
    def test_integer(self):
        ...
        assert_allclose(a1.widths, 1)
        assert_allclose(a1.edges, np.linspace(-0.5, 9.5, 11))
        assert_allclose(a1.pcenters, range(10))
        assert_allclose(a1.pedges, a1.edges)

        a2 = auto_axis(range(10), 20)
        assert_allclose(a2.widths, 1)
        assert_allclose(a2.edges, np.linspace(-0.5, 9.5, 11))
        assert_allclose(a2.pcenters, range(10))
        assert_allclose(a2.pedges, a2.edges)

        a3 = auto_axis(range(20), 10)
        assert_allclose(a3.widths, 2)
        assert_allclose(a3.edges, range(0, 21, 2))
        assert_allclose(a3.pcenters, range(1, 21, 2))
        assert_allclose(a3.pedges, range(0, 21, 2))

        a4 = auto_axis(range(2, 21), 10)
        assert_allclose(a4.widths, 9 * [2] + [1])
        assert_allclose(a4.edges, [*list(range(2, 21, 2)), 21])
        assert_allclose(a4.pcenters, [*list(range(3, 20, 2)), 20.5])
        assert_allclose(a4.pedges, [*list(range(2, 21, 2)), 21])

        a5 = auto_axis(range(3, 22), 10)
        assert_allclose(a5.widths, [1] + 9 * [2])
        assert_allclose(a5.edges, [3, *list(range(4, 23, 2))])
        assert_allclose(a5.pcenters, [3.5, *list(range(5, 22, 2))])
        assert_allclose(a5.pedges, [3, *list(range(4, 23, 2))])
        ...
        assert_allclose(a7.edges, np.arange(9.5, 16))
        assert_allclose(a7.pcenters, range(10, 16))
        assert_allclose(a7.pedges, np.arange(9.5, 16))

        a8 = auto_axis(range(5, 101), bins=10, span=[11, 90])
        print_axis(a8)
        assert_allclose(a8.widths, 8)
        assert_allclose(a8.edges, range(11, 92, 8))
    ...
    def test_log_int_axis(self):
        ...
        assert_allclose(a1.edges, b1 - 0.5)
        assert_allclose(a1.pedges, b1)
        assert_allclose(a1.widths, b1[1:] - b1[:-1])
        assert_allclose(a1.pcenters, (b1[1:] * b1[:-1]) ** 0.5)

        b2 = np.arange(1, 21)
        a2 = auto_axis(b2, 10, t="logint")
        bb = np.array([1, 2, 3, 4, 5, 7, 9, 12, 15, 20])
        assert_allclose(a2.bins, bb)
        assert_allclose(a2.widths, np.diff(bb))
        assert_allclose(a2.edges, bb - 0.5)
        assert_allclose(a2.pedges, bb)
        assert_allclose(a2.pcenters, (bb[1:] * bb[:-1]) ** 0.5)

        a3 = auto_axis(range(10, 101), 10, t="logint")
        b3 = (np.geomspace(10, 100, 10) + 0.5).astype(int)
        assert_allclose(a3.bins, b3)
        assert_allclose(a3.edges, b3 - 0.5)
        assert_allclose(a3.pedges, b3)
        assert_allclose(a3.widths, b3[1:] - b3[:-1])
        assert_allclose(a3.pcenters, (b3[1:] * b3[:-1]) ** 0.5)
    ...
    def test_linear(self):
        ...
        assert_allclose(a1.bins, b1)
        assert_allclose(a1.widths, 10 * [1])
        assert_allclose(a1.edges, b1)
        assert_allclose(a1.pedges, b1)
    ...
    def test_log_axis(self):
        ...
        assert_allclose(a1.bins, b1)
        assert_allclose(a1.widths, np.diff(b1))
        assert_allclose(a1.edges, b1)
        assert_allclose(a1.pedges, b1)
        assert_allclose(a1.pcenters, np.sqrt(b1[:-1] * b1[1:]))
